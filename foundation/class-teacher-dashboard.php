...
final class MathBinder_Teacher_Dashboard {
    ...
    const CLASSES_PAGE_SLUG = 'teacher-classes';
    const STAFF_PAGE_SLUG = 'teacher-class-staff';
    ...
    public static function ensure_page() {
        ...
        foreach ([
            ...
            if($section_page)$section_data['ID']=$section_page->ID;
            wp_insert_post($section_data);
        }
    }
    ...
    public static function enqueue_assets() {
        ...
        if (is_page($teacher_pages)) wp_enqueue_style('mathbinder-teacher-canvas', plugins_url('assets/teacher-canvas.css', __FILE__), ['mathbinder-teacher-dashboard'], MathBinder_Core::VERSION);
        if (is_page($teacher_pages)) wp_enqueue_script('mathbinder-teacher-dashboard', plugins_url('assets/teacher-dashboard.js', __FILE__), [], MathBinder_Core::VERSION, true);
        if (is_page($teacher_pages)) wp_enqueue_style('mathbinder-teacher-dashboard-complete', plugins_url('assets/teacher-dashboard-complete.css', __FILE__), ['mathbinder-teacher-dashboard'], MathBinder_Core::VERSION);
...
PROMPT;
    ...
    public static function handle_canvas_prepare() {
        ...
        wp_safe_redirect(add_query_arg('canvas_notice', 'prepared', home_url('/'.self::CANVAS_PAGE_SLUG.'/'))); exit;
    }
    ...
    public static function handle_mastery_path() {
        ...
        if ($title === '' || $standard === '' || !in_array($grade_level, $allowed_mastery_grades, true) || !$valid_due_date || $threshold < 1 || $threshold > 100 || ($save_mode === 'published' && (in_array('', $publish_fields, true) || in_array('', $branches, true) || !$valid_target || !$lesson_ids))) {
            wp_safe_redirect(add_query_arg('path_notice', 'invalid', home_url('/'.self::MASTERY_PAGE_SLUG.'/'))); exit;
        }
        ...
        wp_safe_redirect(add_query_arg('path_notice', $save_mode === 'published' ? 'published' : 'saved', home_url('/'.self::MASTERY_PAGE_SLUG.'/'))); exit;
    }
    ...
    public static function handle_evidence_review() {
        ...
        if (($decision === 'feedback' || $decision === 'revision_requested') && $feedback === '') {
            wp_safe_redirect(add_query_arg(['student'=>$student_id,'review_notice'=>'feedback_required'], home_url('/'.self::EVIDENCE_PAGE_SLUG.'/'))); exit;
        }
        ...
        wp_safe_redirect(add_query_arg(['student'=>$student_id,'review_notice'=>'saved'], home_url('/'.self::EVIDENCE_PAGE_SLUG.'/'))); exit;
    }
    ...
    private static function render_class_workspace($user, $class, $profiles, $paths) {
        ...
        <div class="mb-teacher-dashboard mb-class-workspace">
            ...
            <section id="students" class="mb-teacher-panel"><div class="mb-teacher-heading"><div><small>Enrollment and progress</small><h2>Students</h2><p><?php echo absint($active_count); ?> active · <?php echo absint($pending_count); ?> pending</p></div></div>
                ...
                <?php foreach($active as $student): $student_url=add_query_arg(['student'=>(int)$student['user_id'],'class_id'=>$class_id],home_url('/'.self::PROGRESS_PAGE_SLUG.'/')); $submitted_note_count=count(MathBinder_Math_Notes::submitted_for_student((int)$student['user_id'],[$class_id]));?><tr><td><strong><a href="<?php echo esc_url($student_url);?>"><?php echo esc_html($student['display_name']?:'Student');?></a></strong><small><?php echo esc_html($student['user_email']);?></small></td><td><span class="mb-roster-status is-active">Active</span></td><td><?php echo absint($student['metrics']['completed']);?> lessons<small><?php echo absint($submitted_note_count);?> submitted notes · <?php echo count($student['assignments']);?> assignments</small></td><td><?php echo $student['metrics']['last']?esc_html(wp_date(get_option('date_format'),strtotime($student['metrics']['last']))):'No activity yet';?></td><td><a class="mb-roster-progress-link" href="<?php echo esc_url($student_url);?>">View full progress and work</a></td></tr><?php endforeach;?>
    ...
    public static function shortcode() {
        ...
        foreach ($rows as $row) if ((int)$row['user_id'] === $selected_id && (!$selected_class_id||(int)$row['class_id']===$selected_class_id) && ($row['can_view_progress'] || $row['can_view_evidence'])) { $selected = $row; break; }
        ...
        <div class="mb-teacher-dashboard mb-view-<?php echo esc_attr($selected ? 'student' : $view); ?>">
            ...
            <?php if ($selected && MathBinder_Class_Staff::can($user->ID,$selected['class_id'],'view_evidence')): echo MathBinder_Math_Notes::teacher_section($selected_id, [(int)$selected['class_id']]); endif; ?>
