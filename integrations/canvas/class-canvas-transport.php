...
final class MathBinder_Canvas_Transport {
    ...
    public static function config(){
        ...
        return ['title'=>'MathBinder','description'=>'California standards-aligned mastery paths, evidence, and progress','oidc_initiation_url'=>$base.'oidc/login','target_link_uri'=>$base.'lti/launch','public_jwk_url'=>$base.'jwks','scopes'=>['https://purl.imsglobal.org/spec/lti-ags/scope/lineitem','https://purl.imsglobal.org/spec/lti-ags/scope/score','https://purl.imsglobal.org/spec/lti-nrps/scope/contextmembership.readonly'],'extensions'=>[['domain'=>(string)wp_parse_url(home_url('/'),PHP_URL_HOST),'tool_id'=>'mathbinder','platform'=>'canvas.instructure.com','settings'=>['text'=>'MathBinder','icon_url'=>'','placements'=>[['placement'=>'course_navigation','message_type'=>'LtiResourceLinkRequest','target_link_uri'=>$base.'lti/launch','text'=>'MathBinder'],['placement'=>'assignment_selection','message_type'=>'LtiDeepLinkingRequest','target_link_uri'=>$base.'lti/launch','text'=>'Select MathBinder Lesson'],['placement'=>'homework_submission','message_type'=>'LtiDeepLinkingRequest','target_link_uri'=>$base.'lti/launch','text'=>'Submit MathBinder Evidence']]]]],'public_jwk'=>json_decode((string)(MathBinder_Canvas_Settings::get()['public_jwk']??'{}'),true)];
    }
    ...
    public static function oidc(WP_REST_Request $request){
        ...
        $s=MathBinder_Canvas_Settings::get();$iss=rtrim(sanitize_text_field((string)$request->get_param('iss')),'/');$login_hint=substr(sanitize_text_field((string)$request->get_param('login_hint')),0,2000);$target=esc_url_raw((string)$request->get_param('target_link_uri'));$client_id=sanitize_text_field((string)$request->get_param('client_id'));
        $expected_issuer=rtrim((string)($s['platform_issuer']??$s['canvas_url']),'/');$launch_url=rest_url(self::REST_NAMESPACE.'/canvas/lti/launch');
        if($iss===''||!hash_equals($expected_issuer,$iss)||$login_hint===''||!hash_equals($launch_url,$target)||($client_id!==''&&!hash_equals((string)$s['client_id'],$client_id)))return new WP_Error('mb_lti_login','Invalid Canvas login initiation.',['status'=>400]);
        $state=MathBinder_Canvas_Crypto::b64url_encode(random_bytes(32));$nonce=MathBinder_Canvas_Crypto::b64url_encode(random_bytes(32));set_transient('mb_lti_state_'.hash('sha256',$state),['nonce'=>$nonce,'target'=>$target,'issuer'=>$iss,'client_id'=>(string)$s['client_id']],10*MINUTE_IN_SECONDS);
        ...
        if(!self::trusted_endpoint($url,$s['canvas_auth_url']))return new WP_Error('mb_lti_authorization_host','Canvas authorization endpoint is not trusted.',['status'=>500]);
        wp_redirect($url,302,'MathBinder Canvas LTI');exit;
    }
