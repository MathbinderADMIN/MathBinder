...
final class MathBinder_Canvas_Settings {
    ...
    public static function render() {
        ...
        <div class="wrap"><h1>MathBinder Canvas Sandbox</h1>
            ...
            <div class="notice <?php echo $s['sandbox_enabled'] ? 'notice-success' : ($complete ? 'notice-info' : 'notice-warning'); ?> inline"><p><strong><?php echo $s['sandbox_enabled'] ? 'Canvas sandbox transmission gate is enabled.' : ($complete ? 'Configuration is complete but transmission is disabled.' : 'Canvas sandbox is not configured.'); ?></strong></p><p>Core 30.51.8 can authenticate approved mapped users inside embedded Canvas lessons, return teacher-selected lessons, accept and record versioned Canvas-linked student assignment snapshots, and send teacher-approved test grades only after every gate and mapping is approved. Production transmission remains disabled until the sandbox certification checklist is completed.</p></div>
    ...
    private static function render_testing(array $tests, array $summary, array $preview, array $history, array $mappings, array $jobs) { ?>
        ...
        <?php if (!$mappings): ?><p>No deployment-scoped mappings are waiting for review.</p><?php else: ?><table class="widefat striped"><thead><tr><th>Type</th><th>External ID hash</th><th>MathBinder target</th><th>Status</th><th>Approval</th><th>Updated</th></tr></thead><tbody><?php foreach ($mappings as $row): ?><tr><td><?php echo esc_html($row['mapping_type']); ?></td><td><code><?php echo esc_html(substr(hash('sha256',(string)$row['external_id']),0,12)); ?></code></td><td><?php echo esc_html($row['mathbinder_type'].' #'.$row['mathbinder_id']); ?></td><td><?php echo esc_html($row['status']); ?></td><td><?php if(in_array($row['mapping_type'],['context','user'],true)): ?><form method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>"><input type="hidden" name="action" value="mathbinder_canvas_approve_mapping"><input type="hidden" name="mapping_id" value="<?php echo absint($row['id']); ?>"><?php wp_nonce_field('mathbinder_canvas_approve_mapping','mathbinder_canvas_mapping_nonce'); ?><label><?php echo $row['mapping_type']==='context'?'MathBinder class ID':'MathBinder user ID'; ?> <input type="number" min="1" name="mathbinder_id" value="<?php echo absint($row['mathbinder_id']); ?>" required></label><?php submit_button($row['status']==='approved'?'Update approved mapping':'Approve mapping','secondary','submit',false); ?></form><?php else: ?>Not applicable<?php endif; ?></td><td><?php echo esc_html($row['updated_at']); ?></td></tr><?php endforeach; ?></tbody></table><?php endif; ?>
