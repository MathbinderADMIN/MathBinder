...
final class MathBinder_Organization_Migrations {
    ...
    public static function run() {
        ...
        dbDelta("CREATE TABLE {$p}mb_enrollments (
            ...
            created_at datetime NOT NULL, updated_at datetime NOT NULL,
            PRIMARY KEY  (id), KEY class_user (class_id,user_id,role_key),
            KEY class_status (class_id,status), KEY invited_email (invited_email)
        ) {$c};");
        // Pending invitations use user_id=0. A unique class/user index therefore
        // blocked every pending student after the first one in a classroom.
        $class_user_index=$wpdb->get_row("SHOW INDEX FROM {$p}mb_enrollments WHERE Key_name='class_user' LIMIT 1",ARRAY_A);
        if($class_user_index && isset($class_user_index['Non_unique']) && (int)$class_user_index['Non_unique']===0){
            $wpdb->query("ALTER TABLE {$p}mb_enrollments DROP INDEX class_user");
            $wpdb->query("ALTER TABLE {$p}mb_enrollments ADD KEY class_user (class_id,user_id,role_key)");
        }
        dbDelta("CREATE TABLE {$p}mb_student_invitations (
            id bigint(20) unsigned NOT NULL AUTO_INCREMENT,
            class_id bigint(20) unsigned NOT NULL,
            ...
            PRIMARY KEY  (id), UNIQUE KEY class_email (class_id,invited_email),
            KEY class_status (class_id,status), KEY email_status (invited_email,status)
        ) {$c};");
