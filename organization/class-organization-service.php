...
final class MathBinder_Organization_Service {
    ...
    public static function enroll($class_id,$email,$role='student') {
        global $wpdb; $class_id=absint($class_id); $email=sanitize_email($email); $role=sanitize_key($role); $user=get_user_by('email',$email); $now=current_time('mysql',true); $uid=$user?(int)$user->ID:0; $status=$user?'active':'invited'; $source=$role==='student'?'teacher_invitation':'administrator'; $approver=get_current_user_id();
        $ok=$wpdb->insert($wpdb->prefix.'mb_enrollments',['class_id'=>$class_id,'user_id'=>$uid,'invited_email'=>$email,'role_key'=>$role,'status'=>$status,'source'=>$source,'approved_by'=>$approver,'created_at'=>$now,'updated_at'=>$now],['%d','%d','%s','%s','%s','%s','%d','%s','%s']);
        if(!$ok && $uid) $ok=$wpdb->update($wpdb->prefix.'mb_enrollments',['status'=>'active','source'=>$source,'approved_by'=>$approver,'updated_at'=>$now],['class_id'=>$class_id,'user_id'=>$uid,'role_key'=>$role]);
        ...
        $mail_sent=null;
        if($role==='student'){
            $class=$wpdb->get_row($wpdb->prepare("SELECT c.name,c.section_name,c.class_code,u.display_name AS teacher_name FROM {$wpdb->prefix}mb_classes c LEFT JOIN {$wpdb->users} u ON u.ID=c.teacher_user_id WHERE c.id=%d",$class_id),ARRAY_A);
            if($class){
                $destination=$user ? add_query_arg('class_id',$class_id,home_url('/student-dashboard/')).'#mb-student-classroom' : add_query_arg('class_code',$class['class_code'],home_url('/student-dashboard/'));
                $subject=$user ? 'You were added to a MathBinder class' : 'You are invited to a MathBinder class';
                $message=sprintf("%s has %s you %s %s in MathBinder.\n\n%s\n%s\n\nUse your own MathBinder login. If your school has not created your account yet, ask your teacher or school administrator for help.", $class['teacher_name']?:'Your teacher', $user?'added':'invited', $user?'to':'to join', trim($class['name'].($class['section_name']?' · '.$class['section_name']:'')), $user?'Open your class:':'Open the invitation:', esc_url_raw($destination));
