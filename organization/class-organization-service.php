...
final class MathBinder_Organization_Service {
    ...
    public static function enroll($class_id,$email,$role='student') { global $wpdb; $class_id=absint($class_id); $email=sanitize_email($email); $role=sanitize_key($role); $user=get_user_by('email',$email); $now=current_time('mysql',true); $uid=$user?(int)$user->ID:0; $status=$user?'active':'invited'; $source=$role==='student'?'teacher_invitation':'administrator'; $approver=get_current_user_id(); $ok=$wpdb->insert($wpdb->prefix.'mb_enrollments',['class_id'=>$class_id,'user_id'=>$uid,'invited_email'=>$email,'role_key'=>$role,'status'=>$status,'source'=>$source,'approved_by'=>$approver,'created_at'=>$now,'updated_at'=>$now],['%d','%d','%s','%s','%s','%s','%d','%s','%s']); if(!$ok && $uid) $wpdb->update($wpdb->prefix.'mb_enrollments',['status'=>'active','source'=>$source,'approved_by'=>$approver,'updated_at'=>$now],['class_id'=>$class_id,'user_id'=>$uid,'role_key'=>$role]); if($user) { MathBinder_Identity_Service::assign_role($uid,$role,'class',$class_id); if($role==='student'){ $organization_id=absint($wpdb->get_var($wpdb->prepare("SELECT organization_id FROM {$wpdb->prefix}mb_classes WHERE id=%d",$class_id))); MathBinder_Verification_Service::authorize_minor($uid,0,'','school',$organization_id); } } MathBinder_Audit_Log::record('enroll_user','class',$class_id,['user_id'=>$uid,'email'=>$email,'status'=>$status,'coverage_candidate'=>$role==='student']); return true; }
