...
final class MathBinder_Identity_Service {
    ...
    public static function on_login($login, $user) {
        ...
        self::provision_profile($user->ID);
        self::accept_pending_student_invitations($user);
    }

    private static function accept_pending_student_invitations($user) {
        global $wpdb;
        $email = strtolower(sanitize_email($user->user_email));
        if (!$email) return;
        $invitations = $wpdb->get_results($wpdb->prepare(
            "SELECT i.id,i.class_id,c.organization_id
             FROM {$wpdb->prefix}mb_student_invitations i
             INNER JOIN {$wpdb->prefix}mb_classes c ON c.id=i.class_id AND c.status='active'
             WHERE i.invited_email=%s AND i.status='invited'",
            $email
        ), ARRAY_A) ?: [];
        if (!$invitations) return;
        $now = current_time('mysql', true);
        foreach ($invitations as $invitation) {
            $class_id = (int)$invitation['class_id'];
            $existing = $wpdb->get_row($wpdb->prepare(
                "SELECT id FROM {$wpdb->prefix}mb_enrollments WHERE class_id=%d AND role_key='student' AND (user_id=%d OR invited_email=%s) ORDER BY id DESC LIMIT 1",
                $class_id, $user->ID, $email
            ), ARRAY_A);
            $data = ['user_id'=>$user->ID,'invited_email'=>$email,'role_key'=>'student','status'=>'active','source'=>'accepted_invitation','approved_by'=>0,'updated_at'=>$now];
            if ($existing) $wpdb->update($wpdb->prefix.'mb_enrollments',$data,['id'=>(int)$existing['id']]);
            else { $data['class_id']=$class_id; $data['created_at']=$now; $wpdb->insert($wpdb->prefix.'mb_enrollments',$data); }
            $wpdb->update($wpdb->prefix.'mb_student_invitations',['status'=>'accepted','accepted_at'=>$now,'updated_at'=>$now],['id'=>(int)$invitation['id']],['%s','%s','%s'],['%d']);
            self::assign_role($user->ID,'student','class',$class_id,'active','accepted_invitation');
            MathBinder_Verification_Service::authorize_minor($user->ID,0,'','school',(int)$invitation['organization_id']);
            MathBinder_Audit_Log::record('accept_student_invitation','class',$class_id,['user_id'=>$user->ID,'email'=>$email]);
        }
        update_user_meta($user->ID,'mb_account_activation_status','active');
        update_user_meta($user->ID,'mb_first_login_completed_at',$now);
    }
